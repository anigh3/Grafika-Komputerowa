...
void GenerateTextures()
{
	...
	GLboolean error = load_targa("white skin guy black hairs.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku white skin guy black hairs.tga");
		exit(0);
	}
	...
	error = load_targa("white skin guy black hairs_gray.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku white skin guy black hairs_gray.tga");
		exit(0);
	}
