...
// właściwości materiału - domyślnie miedź

const GLfloat *ambient = CopperAmbient;
const GLfloat *diffuse = CopperDiffuse;
const GLfloat *specular = CopperSpecular;
GLfloat shininess = CopperShininess;
...
int main(int argc, char *argv[])
{
	...
	//utworzenie podmenu - światło otaczające, kierunkowe
	int MenuSwiatlo = glutCreateMenu(Menu);
#ifndef WIN32
	glutAddMenuEntry("Swiatlo otaczajace", OTACZAJACE);
#else

	glutAddMenuEntry("Swiatlo otaczajace", OTACZAJACE);
	glutAddMenuEntry("Swiatlo kierunkowe", KIERUNKOWE);
